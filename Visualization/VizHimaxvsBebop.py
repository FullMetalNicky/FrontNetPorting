...
def VizDroneBEV(frames, pred_bebop, pred_himax):
    ...
    ani.save('updownEq.mp4', writer=writer)
    #ani.save('himaxVsbebop.gif', dpi=80, writer='imagemagick')
    plt.show()


def HistogtamEqual(origImgs):
    eqlImgs = []
    for img in origImgs:
...
def InferenceData(trainer):
    images = ImageIO.ReadImagesFromFolder("../data/updown/himax_processed/", '.jpg', 0)
    images = HistogtamEqual(images)
    [x_live, y_live] = DataProcessor.ProcessInferenceData(images, 60, 108, True)
    ...
    images = ImageIO.ReadImagesFromFolder("../data/updown/bebop_processed/", '.jpg', 0)
    images = HistogtamEqual(images)
    [x_live, y_live] = DataProcessor.ProcessInferenceData(images, 60, 108, True)
    live_set = Dataset(x_live, y_live)
    params = {'batch_size': 1,
