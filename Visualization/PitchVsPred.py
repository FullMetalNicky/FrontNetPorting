...
def PlotModelPred(ax, y_values, y_gt, x_values,  x_labels, len, skip, color, title, model_label, lim):
    ...
    ax.plot(x_values, y_gt, color='k', label='GT')
    ax.set_title(title)
...
def VizPitchvsPredSubPlots(ax, outputs, gt_labels, p_test, color, model_label):
   ...
   # outputs = np.reshape(outputs, (-1, range_p, 4))
    ...
    PlotModelPred(ax[0][0], outputs[:, 0], gt_labels[:, 0], p_test, pitch_labels, len_labels, skip, color, "x", model_label, [1, 2])
    PlotModelPred(ax[0][1], outputs[:, 1], gt_labels[:, 1], p_test, pitch_labels, len_labels, skip, color, "y", model_label, [-0.5, 0.5])
    PlotModelPred(ax[1][0], outputs[:, 2], gt_labels[:, 2], p_test, pitch_labels, len_labels, skip, color, "z", model_label, [0, 0.4])
    PlotModelPred(ax[1][1], outputs[:, 3], gt_labels[:, 3], p_test, pitch_labels, len_labels, skip, color, "phi", model_label, [-0.6, 0.6])
...
def Plot1Model(p_test, name):
    ...
    name1 = "pickles/DronetOthes160x90StaticPitchResults.pickle"
    outputs, gt_labels = LoadPerformanceResults(name1)
